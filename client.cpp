 ...
 * @brief 获取摄像头视频文件 api
 *
 * 该api使用libssh2库的SCP协议从远程服务器下载文件
 *
 * @param args json格式的字符串，包含以下字段:
 *   {
 *       "host": "your_host",
 *       "port": "22",
...
bool Client::get_camera_video(const std::string& args, ResponseHandler handler)
{
    ...
    if (!jargs.contains("host") || !jargs["host"].is_string() ||
        !jargs.contains("username") || !jargs["username"].is_string() ||
        !jargs.contains("password") || !jargs["password"].is_string() ||
        !jargs.contains("remote_file") || !jargs["remote_file"].is_string() ||
        (jargs.contains("port") && !jargs["port"].is_string())) {
        log_error("invalid argument field, please check your input");
        return false;
    }
    ...
    std::thread download_thread([this, handler, host, port, username, passwd, remote_file, local_file]() {
        ...
        if (scp_success) {
            ...
            std::string error_message = scp_success.error_message;
            std::cerr << "download file with ssh2 failed: " << error_message << std::endl;
            io_context_.post([this, handler, error_message]() {
                handler(get_response(ERROR_CODE::GET_FILE_FAILED, error_message));
            });
        }
...
/**
 * @brief 使用libssh2库通过SCP协议从远程服务器下载文件（优化简化版）
 ...
 * @return Client::DownloadResult 包含成功状态和错误信息的结果对象
 */
Client::DownloadResult Client::download_file_ssh(const std::string& host, const std::string& port,
    ...
    const std::string& remote_file, const std::string& local_file)
{
    ...
    try {
        ...
        if (WSAStartup(MAKEWORD(2, 0), &wsadata) != 0) {
            return {false, "Failed to initialize Winsock"};
        }
        #endif
        ...
        // 打开本地文件
        // w+: 读写模式，如果文件不存在则创建，反之清空文件
        fp = fopen(local_file.c_str(), "w+b");
        if (!fp) {
        ...
        /*struct stat*/libssh2_struct_stat fileinfo;
        const char* scp_path = remote_file.c_str();

        // 使用SCP下载文件
        channel = libssh2_scp_recv2(session, scp_path, &fileinfo);
        char* errmsg = nullptr;
        int errmsg_len = 0;
        if (!channel) {
            // 如果SCP失败，直接返回
            libssh2_session_last_error(session, &errmsg, &errmsg_len, 0);
            error_message = "Unable to open SCP channel: ";
            error_message += errmsg ? errmsg : "unknown error";
            throw std::runtime_error(error_message);
        }

        // SCP传输
        std::cerr << "Using SCP protocol, file size: " << fileinfo.st_size << " bytes" << std::endl;

        // 128kb
        char buffer[131072];
        size_t total_received = 0;
        while (total_received < static_cast<size_t>(fileinfo.st_size)) {
            // 计算剩余需要读取的字节数
            int remaining = fileinfo.st_size - total_received;
            // 不读取超过剩余字节的数据
            int to_read = (remaining < sizeof(buffer)) ? remaining : sizeof(buffer);
            int rc = libssh2_channel_read(channel, buffer, to_read);
            if (rc > 0) {
                if (fwrite(buffer, 1, rc, fp) != static_cast<size_t>(rc)) {
                ...
                if (fileinfo.st_size > 0) {
                    int percent = (total_received * 100) / fileinfo.st_size;
                    std::cerr << "\rDownloading: " << percent << "% complete" << std::flush;
                }
            } else if (rc < 0) {
                throw std::runtime_error("Error reading from remote file");
            }
        }
        std::cerr << std::endl << "Download complete" << std::endl;
        success = true;
    }
